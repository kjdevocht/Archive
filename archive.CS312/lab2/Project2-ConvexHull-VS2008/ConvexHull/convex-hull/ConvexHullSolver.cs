...
namespace _2_convex_hull
{
    class ConvexHullSolver
    {
        ...
        public void Solve(System.Drawing.Graphics g, List<System.Drawing.PointF> pointList)
        {
            ...
            List<System.Drawing.PointF> hull = dividePoints(pointList);
            Pen pen = new Pen(Color.Red, 3);
            System.Drawing.PointF [] hull2 = hull.ToArray();
            g.DrawPolygon(pen, hull2);
        }
        ...
        List<System.Drawing.PointF> dividePoints(List<System.Drawing.PointF> Q)
        {
            ...
            if(LQ.Count>1 && RQ.Count>1)
            {
                ...
                if (i != LQ.Count-1)//this means that lq and lq+1 have the same y value
                {
                    if(isAbove(LQ[i + 1], LQ[i], RQ[j]))
                    {
                    newQ.Add(LQ[i]);
                    ...
                    for(int k = j; k<RQ.Count-1; k++) //looking for the top common tangent 
                    {
                        if (j != RQ.Count - 1)
                        {
                            if(isAbove(RQ[j + 1], LQ[i], RQ[j]))
                            {
                ...
                for(int k = j; k<RQ.Count-1; k++) 
                {
                    if (j != RQ.Count - 1)
                    {
                        ...
                        {
                            newQ.Add(RQ[j]);
                            j++;//because it is bigger it cannot be the lowest point so add it as part of the right side of the hull
                        }
                ...
                if (j != RQ.Count-1)//this means that RQ and RQ+1 have the same y value
                {
                    if(isBelow(RQ[j + 1], LQ[i], RQ[j]))
                    {
                        newQ.Add(RQ[i]);
                        newQ.Add(RQ[i + 1]);//Both of these points must be in the hull because the left side is lower then the right
                        i++;//update i to point at the point on the left hull used in the top common tangent
                        for (int k = i; k < RQ.Count - 1; k++)   //looking for the top common tangent 
                        {
                            if (isBelow(LQ[i + 1], LQ[i], RQ[j]))
                            {
