    ...
    def download(self):
        try:
            ...
            if r.headers['content-length'] > 0:
                ...
                threadSizes[-1]+=leftOvers
                threads = []
                t0 = time.clock()
                for size in range(len(threadSizes)):
                    if size == 0:
                        temp = threadSizes[size]-1
                        d = DownThread(self.url, temp, self.byteIndex)
                        self.byteIndex += threadSizes[size]
                    else:
                        d = DownThread(self.url,threadSizes[size]-1, self.byteIndex)
                        self.byteIndex += threadSizes[size]
                    threads.append(d)
                for t in threads:
                    t.start()
