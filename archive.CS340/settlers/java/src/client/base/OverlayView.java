...
public class OverlayView extends PanelView implements IOverlayView
{
	...
	public void showModal()
	{
        ...
        OverlayInfo overlayInfo = new OverlayInfo(this, overlayPanel);
		overlayStack.push(overlayInfo);
        modal = overlayInfo;
	}
...
//		assert window.getGlassPane() == overlayStack.peek().getOverlayPanel();
		...
		{
            if(modal != null) {
                while(overlayStack.contains(modal)) {
                    modal.getOverlayPanel().setVisible(false);
                    overlayStack.remove(modal);
                }
                modal = null;
            }
            else {
