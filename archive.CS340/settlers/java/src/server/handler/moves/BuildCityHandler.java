...
/**
 * Server Handler to process /moves/buildCity
 */
public class BuildCityHandler implements IHandler {
    @Override
    public void handle(HttpExchange httpExchange) throws IOException {
        Logger.getSingleton().info("Build City Handler");
        OutputStream os = httpExchange.getResponseBody();
        String result = "";
        Logger.getSingleton().debug(httpExchange.getRequestMethod());
        ...
        if(!(httpExchange.getRequestMethod().equals("POST") || httpExchange.getRequestMethod().equals("PUT"))) {
            ...
            try{
                ...
                BuildCityRequest request = translator.getBuildCityRequest();
                IGameModel gameModel = new ModelResult(ServerFacade.getSingleton().buildCity(request, credentials)).getResult();
                if(gameModel == null) {
