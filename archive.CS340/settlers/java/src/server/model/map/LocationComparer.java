...
public class LocationComparer {
    ...
    public static List<VertexLocation> getVertexLocations(VertexLocation vertex) {
        vertex = vertex.getNormalizedLocation();
        List<VertexLocation> list = new ArrayList<VertexLocation>();
        if(vertex.getDir() ==  VertexDirection.NorthWest) {
            HexLocation hex = vertex.getHexLoc();
            list.add(vertex);
            list.add(new VertexLocation(new HexLocation(hex.getX()-1, hex.getY()), VertexDirection.East));
            list.add(new VertexLocation(new HexLocation(hex.getX(), hex.getY()-1), VertexDirection.SouthWest));
        }
        else if(vertex.getDir() ==  VertexDirection.NorthEast) {
            HexLocation hex = vertex.getHexLoc();
            list.add(vertex);
            list.add(new VertexLocation(new HexLocation(hex.getX(), hex.getY()-1), VertexDirection.SouthEast));
            list.add(new VertexLocation(new HexLocation(hex.getX()+1, hex.getY()-1), VertexDirection.West));
        }
        return list;
    }
}
