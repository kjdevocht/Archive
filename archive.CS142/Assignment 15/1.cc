/*Kevin DeVocht assignment 15 exercise 1. Develop the function howFarRemoved. 
It determines how far a blue-eyed descendant, if one exists, is removed from 
the given parent. If the given parent has blue eyes, the distance is 0; if eyes 
is not blue but at least one its children’s eyes are, the distance is 1; and so on. 
If no descendant of the given parent has blue eyes, the function returns infinity 
...
int main () {
  ...
  /*Substitution
	1. AragornD->howFarRemoved()
	
	2. new Person( AragornI, (new oneDFT( EldarionD, (new mtLoDFTs()))))->howFarRemoved()
	
	3. if (this->info->howFarRemoved() == 0){return 0;}else {return this->info->howFarRemoved() + this->children->howFarRemoved();
	
	4. if (new Person( AragornI, (new oneDFT( EldarionD, (new mtLoDFTs()))))->info->howFarRemoved() == 0){return 0;}else {return new Person( AragornI, (new oneDFT( EldarionD, (new mtLoDFTs()))))->info->howFarRemoved() + new Person( AragornI, (new oneDFT( EldarionD, (new mtLoDFTs()))))->children->howFarRemoved();
	
	5. if (AragornI->howFarRemoved() == 0){return 0;}else {return AragornI->howFarRemoved() + EldarionD->howFarRemoved(), new mtLoDFTs()->howFarRemoved();	
*/	
}