...
string parse(vector<Token*> results)
{
	...
	cout<<"Before" + tok.tokenList.size()<<endl;
	stripComments(tok);
	cout<<"After" + tok.tokenList.size()<<endl;
	Parser parse(tok);
	string parseResults;
	DatalogProgram dlp;
	...
	try
	{
		dlp = parse.dataLogProgramParser();
		parseResults = dlp.toString();
	}
	catch (int e)
	{
		tok.currentToken = e;
		parseResults = "Failure!\n\t(" + tok.getType() + "," + "\"" + tok.getValue() + "\"" + "," + tok.printLineNum() + ")\n";
	}
	return parseResults;	
}
...
int main(int argc, char* argv[]) {
    Lex lex(argv[1]);
	string test = parse(lex.getTokens());
    cout<<test;
    return 0;
}
