...
class Schema: public vector<Parameter>
{
...
class Relation
{
	...
	void joinNameSchema(Relation& qryRltn)
	{
		...
		{
			checkDup.insert(schema.at(i).param);
		}
		
		
		for(int i = 0; i<qryRltn.schema.size(); i++)
		{
			if(checkDup.count(qryRltn.schema.at(i).param) == 0)
			{
	...
	vector<string>& qryTuple, vector<string>& thisTuple)
	{
			for(int i = 0; i<thisSchema.size(); i++)
			{
				...
				{
					if(thisSchema.at(i).param == qrySchema.at(j).param && 
	...
	vector<string>& qryTuple, vector<string>& thisTuple)
	{
			...
			for(int i = 0; i<thisTuple.size(); i++)
			{
				...
				checkDup.insert(thisSchema.at(i).param);
			}
			
			for(int i = 0; i<qryTuple.size(); i++)
			{
				
				if(checkDup.count(qrySchema.at(i).param) == 0)
				{
	...
	string toString()
	{
		...
		else
		{
			...
			{	
				...
				{
					results += schema.at(i).param + "=" + row.at(i);
	...
	void rename(const Predicate& qry, vector<int>& projectLst)
	{
		...
		{
			newSchema.push_back(qry.paramLst.at(projectLst.at(i)));
		}
...
class Database
{
	...
	Schema ruleSchema(vector<Parameter>& paramLst, int& i)
	{
		...
		{
			schema.push_back(paramLst.at(q));			
		}
		return schema;	
	}
	...
	vector<int> createProjectList(Schema schema, Relation& jnRltn)
	{
		...
		for(int j = 0; j<schema.size(); j++)
		{
			...
			{
				if(schema.at(j).param == jnRltn.schema.at(k).param)
				{
	...
	void generateTables(const SchemeList& schmLst, const FactList& fctLst)
	{
		...
		{
			Schema schma;
			for(int j = 0; j<schmLst.schmLst.at(i).scheme.paramLst.size();  j++)
			{
				schma.push_back(schmLst.schmLst.at(i).scheme.paramLst.at(j));
			}
			Relation relation(schmLst.schmLst.at(i).scheme.id, schma);
		...
		for(int j = 0; j<fctLst.fctLst.size(); j++)
		{
			...
			Predicate fct = facts.at(j).fact;
			Tuple tuple;
			for(int j = 0; j<fct.paramLst.size(); j++)
			{
				tuple.push_back(fct.paramLst.at(j).param);
			}
	...
	void readInput()
	{
		...
		print(endResults, this->output, count);	
	}
	
	void print(string results, char* output, int& count)
	{
		string total;
		stringstream streamin(stringstream::out);
		streamin<<count;
