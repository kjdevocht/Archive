...
class Database
{
	...
	void generateTables(SchemeList schmLst, FactList fctLst)
	{
		...
		for(int i = 0; i< schmLst.schmLst.size(); i++)
		{
			...
			Predicate schm = schemes.at(i).scheme;
			Schema schma;
			for(int j = 0; j<schm.paramLst.size();  j++)
			{
				
				schma.push_back(schm.paramLst.at(j));
			}
	...
	string readInput()
	{
		...
		Token end("EOF", "End of File", lineNum);
		results.push_back(end);

		if(results.at(0).value != "ERROR")
		{
			endResults = parse(results);
			string temp;
			it = endResults.begin();
			temp = *it;
			if(temp != "F")
